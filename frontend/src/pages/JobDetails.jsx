...
const JobDetails = () => {
  ...
  const { mutate: applyJob, isLoading: applyingJob } = useMutation({
    ...
    onSuccess: () => {
      toast.success('Your profile has been shared with the recruiter!');
      queryClient.invalidateQueries({ queryKey: ['job', jobId] });
    },
    onError: (err) => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 md:p-10 bg-white rounded-2xl shadow-2xl my-10">
      {/* Job Header */}
      <header className="mb-10 border-b-2 pb-6">
        <h1 className="text-4xl font-bold text-gray-800 mb-2">{job.title}</h1>
        <div className="space-y-1">
          <p className="text-2xl font-semibold text-gray-700">{job.company}</p>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold text-blue-600 mb-4">Job Description</h2>
        <p className="text-gray-700 leading-relaxed text-justify">{job.description}</p>
      </section>
      ...
      {job.postedBy && (
        <section className="mb-10">
          <h2 className="text-2xl font-semibold text-blue-600 mb-4">Posted By</h2>
          <Link to={`/profile/${job.postedBy.username}`} className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg hover:shadow-md transition">
            <img
              src={job.postedBy.profilePicture || '/avatar.png'}
              alt={job.postedBy.name}
      ...
      {/* Applicants Section */}
      <section className="mb-10">
        <h2 className="text-2xl font-semibold text-blue-600 mb-4">Applicants</h2>
        {job.applicants?.length > 0 ? (
          <div className="space-y-4">
            {job.applicants.map((applicant) => (
              <div key={applicant._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:shadow-md transition">
                <Link to={`/profile/${applicant.username}`} className="flex items-center gap-3">
                  <img
                    src={applicant.profilePicture || '/avatar.png'}
                    alt={applicant.name}
      ...
      <div>
        ...
        >
          {applyingJob ? 'Applying...' : 'Apply for this Job'}
        </button>
      </div>
    </div>
  );
